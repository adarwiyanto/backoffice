...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($act==='check_pairing' || $act==='auto_check_pairing'){
    $id=(int)($_POST['id']??0);
    $isAuto=($act==='auto_check_pairing');
    $r=bo_exec('SELECT * FROM bo_pairing_requests WHERE id=?',[$id])->fetch();
    if(!$r){
      if($isAuto){ header('Content-Type: application/json; charset=utf-8'); echo json_encode(['ok'=>false,'done'=>false,'message'=>'Request pairing tidak ditemukan.']); exit; }
      bo_integration_redirect('Request pairing tidak ditemukan.','error','pairingModal');
    }
    ...
    try {
      ...
      bo_exec("UPDATE bo_pairing_requests SET status='approved_processing',message=?,updated_at=NOW() WHERE id=?",['Token sudah tersimpan. Finalisasi otomatis akan dicoba kembali: '.$finalizeError->getMessage(),$id]);
      if($isAuto){ header('Content-Type: application/json; charset=utf-8'); echo json_encode(['ok'=>false,'done'=>false,'status'=>'approved_processing','message'=>$finalizeError->getMessage()]); exit; }
      throw $finalizeError;
    }
    if($isAuto){ header('Content-Type: application/json; charset=utf-8'); echo json_encode(['ok'=>true,'done'=>true,'status'=>'approved','message'=>'Pairing selesai otomatis dan koneksi aktif.']); exit; }
...
<script src="assets/js/integration-pairing-auto.js?v=20260710"></script>
