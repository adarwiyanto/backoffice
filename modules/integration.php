...
function bo_scope_for_target(string $target): string { return 'superadmin'; }
$msg='';
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 if($act==='request_pairing'){
  ...
  if($url==='') $msg='URL tujuan wajib diisi.'; else {
    ...
    bo_exec('INSERT INTO bo_pairing_requests(target_system,target_name,target_base_url,request_code,request_secret,requester_name,requester_type,requested_scope,status,message,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,NOW())',[$target,$name,$url,$code,$secret,'Back Office','backoffice','superadmin',!empty($res['ok'])?'pending':'failed',$res['message']??'']);
