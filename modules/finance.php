...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try{
    ...
    if($action==='save_payroll_employee'){
      ...
      bo_exec("INSERT INTO bo_payroll_employee_settings(assignment_id,bo_user_id,base_salary,allowance,overtime,punishment,kasbon,deduction,adjustment,is_eligible,notes,created_at) VALUES(?,?,?,0,?,?,?,?,0,?,?,NOW()) ON DUPLICATE KEY UPDATE base_salary=VALUES(base_salary),allowance=0,overtime=VALUES(overtime),punishment=VALUES(punishment),kasbon=VALUES(kasbon),deduction=VALUES(deduction),adjustment=0,is_eligible=VALUES(is_eligible),notes=VALUES(notes),updated_at=NOW()",[$assignmentId>0?$assignmentId:null,$boUserId>0?$boUserId:null,$base,$overtime,$punishment,$kasbon,$punishment+$kasbon,isset($_POST['is_eligible'])?1:0,bo_fin_post('notes')]);
...
<?php
$payrollReady=bo_payroll_schema_ready();$payrollUnits=$payrollReady?bo_payroll_unit_settings():[];$payrollEmpSettings=$payrollReady?bo_payroll_employee_settings():[];
...
?>
<?php if(!$payrollReady): ?><div class="alert danger">Import <b>db/20260828_002_kpi_payroll_revision.sql</b> terlebih dahulu.</div><?php else: ?>
<div class="grid">
