...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try{
    ...
    if($action==='save_payroll_employee'){
      ...
      $bpjsKesehatan=(float)str_replace(',','.',bo_fin_post('bpjs_kesehatan','0'));
      $bpjsKetenagakerjaan=(float)str_replace(',','.',bo_fin_post('bpjs_ketenagakerjaan','0'));
      $punishment=(float)str_replace(',','.',bo_fin_post('punishment','0'));
      $kasbon=(float)str_replace(',','.',bo_fin_post('kasbon','0'));
      if(min($base,$overtime,$bpjsKesehatan,$bpjsKetenagakerjaan,$punishment,$kasbon)<0) throw new RuntimeException('Komponen payroll tidak boleh negatif.');
      foreach(['Gaji pokok'=>$base,'Lembur'=>$overtime,'BPJS Kesehatan'=>$bpjsKesehatan,'BPJS Ketenagakerjaan'=>$bpjsKetenagakerjaan,'Punishment'=>$punishment,'Kasbon'=>$kasbon] as $label=>$nominal){
        if(floor($nominal)!=$nominal) throw new RuntimeException($label.' harus berupa rupiah bulat tanpa pecahan desimal.');
      }
      bo_payroll_seed_period($month);
      bo_exec("INSERT INTO bo_payroll_employee_period_settings(payroll_month,assignment_id,bo_user_id,base_salary,overtime,bpjs_kesehatan,bpjs_ketenagakerjaan,punishment,kasbon,is_eligible,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE base_salary=VALUES(base_salary),overtime=VALUES(overtime),bpjs_kesehatan=VALUES(bpjs_kesehatan),bpjs_ketenagakerjaan=VALUES(bpjs_ketenagakerjaan),punishment=VALUES(punishment),kasbon=VALUES(kasbon),is_eligible=VALUES(is_eligible),notes=VALUES(notes),seeded_from_month=NULL,updated_at=NOW()",[$month,$assignmentId>0?$assignmentId:null,$boUserId>0?$boUserId:null,$base,$overtime,$bpjsKesehatan,$bpjsKetenagakerjaan,$punishment,$kasbon,isset($_POST['is_eligible'])?1:0,bo_fin_post('notes')]);
...
<div class="grid">
 ...
 <div class="card metric"><div class="label">Take Home Pay</div><div class="value"><?=money_id($payrollLocked?($payrollRun['total_take_home']??0):($payrollCalc['totals']['take_home']??0))?></div><div class="sub">Gaji pokok + lembur + insentif − BPJS Kesehatan − BPJS Ketenagakerjaan − punishment − kasbon</div></div>
</div>
...
<?php foreach($payrollUnits as $u): $formId='pu-'.preg_replace('/[^a-zA-Z0-9_-]/','-',(string)$u['unit_type'].'-'.(string)$u['system_key']);$basis=0;if($payrollCalc){if($u['unit_type']==='store')$basis=(float)($payrollCalc['financial']['store'][$u['system_key']]['revenue']??0);elseif($u['unit_type']==='dapur')$basis=(float)($payrollCalc['financial']['dapur'][$u['system_key']]['revenue']??0);else $basis=(float)($payrollCalc['financial']['backoffice_revenue']??0);}$budget=$basis*(float)$u['pool_percentage']/100; ?><tr><td><?=e(strtoupper($u['unit_type']))?><form id="<?=e($formId)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_unit"><input type="hidden" name="unit_type" value="<?=e($u['unit_type'])?>"><input type="hidden" name="system_key" value="<?=e($u['system_key'])?>"><input type="hidden" name="unit_name" value="<?=e($u['unit_name'])?>"></form></td><td><b><?=e($u['unit_name'])?></b><br><small><?=e($u['system_key'])?></small></td><td><?=money_id($basis)?><?php if($payrollCalc&&$u['unit_type']==='store'&&!empty($payrollCalc['financial']['store'][$u['system_key']]['revenue_adjustments']['deductions'])):$adj=$payrollCalc['financial']['store'][$u['system_key']]['revenue_adjustments']; ?><br><small>Omset bruto <?=money_id($adj['gross'])?><?php foreach($adj['deductions'] as $cat=>$amt): ?> · − <?=e(ucwords($cat))?> <?=money_id($amt)?><?php endforeach; ?></small><?php endif; ?></td><td><input form="<?=e($formId)?>" type="number" name="pool_percentage" min="0" max="100" step="0.01" value="<?=e($u['pool_percentage'])?>"></td><td><b><?=money_id($budget)?></b></td><td><label><input form="<?=e($formId)?>" style="width:auto" type="checkbox" name="is_active" <?=!empty($u['is_active'])?'checked':''?>> Aktif</label></td><td><button form="<?=e($formId)?>" class="btn">Simpan</button></td></tr><?php endforeach; ?></tbody></table></div></div>

<div class="card section"><h3>Komponen Payroll Pegawai Toko & Dapur</h3><div class="muted">KPI Toko diinput melalui SDM → KPI Toko. KPI Dapur tetap sinkron dari website Dapur.</div><div class="table-wrap payroll-scroll"><table><thead><tr><th>Pegawai</th><th>Unit / Role</th><th>Gaji Pokok</th><th>Lembur</th><th>BPJS Kesehatan</th><th>BPJS Ketenagakerjaan</th><th>Punishment</th><th>Kasbon</th><th>KPI Dapur</th><th>Eligible</th><th></th></tr></thead><tbody>
<?php foreach($payrollAssignments as $a): $es=$payrollEmpSettings['assignment:'.(int)$a['assignment_id']]??[];$isDapur=(string)$a['source_system']==='dapur';$target=$isDapur?bo_payroll_target($month,'dapur',(string)$a['system_key'],(string)$a['role_key']):100;$formId='pe-'.(int)$a['assignment_id']; ?><tr><td><b><?=e($a['canonical_name'])?></b><br><small><?=e($a['external_employee_id'])?></small><form id="<?=e($formId)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_employee"><input type="hidden" name="assignment_id" value="<?=e($a['assignment_id'])?>"><input type="hidden" name="unit_type" value="<?=$isDapur?'dapur':'store'?>"><input type="hidden" name="system_key" value="<?=e($a['system_key'])?>"><input type="hidden" name="role_key" value="<?=e($a['role_key'])?>"></form></td><td><?=e($a['system_name'])?><br><small><?=e($a['role_label']?:$a['role_key'])?></small></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="base_salary" value="<?=e($es['base_salary']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="overtime" value="<?=e($es['overtime']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="bpjs_kesehatan" value="<?=e($es['bpjs_kesehatan']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="bpjs_ketenagakerjaan" value="<?=e($es['bpjs_ketenagakerjaan']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="punishment" value="<?=e($es['punishment']??$es['deduction']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1" name="kasbon" value="<?=e($es['kasbon']??0)?>"></td><td><?php if($isDapur): ?><span class="badge">Bobot 25%</span><br><small>poin sinkron dapur</small><?php else: ?><span class="muted">KPI lokal BO</span><?php endif; ?></td><td><input form="<?=e($formId)?>" style="width:auto" type="checkbox" name="is_eligible" <?=!isset($es['is_eligible'])||!empty($es['is_eligible'])?'checked':''?>></td><td><button form="<?=e($formId)?>" class="btn">Simpan</button></td></tr><?php endforeach;if(!$payrollAssignments): ?><tr><td colspan="11">Belum ada assignment pegawai aktif. Jalankan Sync Pegawai terlebih dahulu.</td></tr><?php endif; ?></tbody></table></div></div>

<div class="card section"><h3>Admin Back Office</h3><div class="muted">KPI Admin tidak lagi diinput di Penggajian. Gunakan SDM → KPI → KPI Back Office. Owner tetap tidak masuk otomatis.</div><div class="table-wrap payroll-scroll"><table><thead><tr><th>Admin</th><th>Gaji Pokok</th><th>Lembur</th><th>BPJS Kesehatan</th><th>BPJS Ketenagakerjaan</th><th>Punishment</th><th>Kasbon</th><th>Eligible</th><th>Aksi</th></tr></thead><tbody>
<?php foreach($payrollAdmins as $a):$es=$payrollEmpSettings['admin:'.(int)$a['id']]??[];$gForm='pa-'.(int)$a['id']; ?><tr><td><b><?=e($a['name'])?></b><br><small><?=e($a['username'])?></small><form id="<?=e($gForm)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_employee"><input type="hidden" name="bo_user_id" value="<?=e($a['id'])?>"></form></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="base_salary" value="<?=e($es['base_salary']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="overtime" value="<?=e($es['overtime']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="bpjs_kesehatan" value="<?=e($es['bpjs_kesehatan']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="bpjs_ketenagakerjaan" value="<?=e($es['bpjs_ketenagakerjaan']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="punishment" value="<?=e($es['punishment']??$es['deduction']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1" name="kasbon" value="<?=e($es['kasbon']??0)?>"></td><td><input form="<?=e($gForm)?>" style="width:auto" type="checkbox" name="is_eligible" <?=!isset($es['is_eligible'])||!empty($es['is_eligible'])?'checked':''?>></td><td><button form="<?=e($gForm)?>" class="btn">Simpan Gaji</button></td></tr><?php endforeach;if(!$payrollAdmins): ?><tr><td colspan="9">Belum ada user role Admin aktif.</td></tr><?php endif; ?></tbody></table></div></div>
<?php endif; ?>
...
<div class="table-wrap payroll-scroll"><table><thead><tr><th>Pegawai</th><th>Unit</th><th>Basis Omset</th><th>% Payroll</th><th>KPI</th><th>Bobot Insentif</th><th>Insentif</th><th>Gaji Pokok</th><th>Lembur</th><th>BPJS Kesehatan</th><th>BPJS Ketenagakerjaan</th><th>Punishment</th><th>Kasbon</th><th>Take Home Pay</th></tr></thead><tbody>
<?php foreach($payrollItems as $it): $snap=$payrollLocked; ?><tr><td><b><?=e($snap?$it['employee_name_snapshot']:$it['employee_name'])?></b><br><small><?=e($snap?$it['role_key_snapshot']:$it['role_key'])?></small></td><td><?=e($snap?$it['unit_name_snapshot']:$it['unit_name'])?><br><small><?=e($it['unit_type'])?></small></td><td><?=money_id($it['revenue_base']??0)?></td><td><?=e(number_format((float)($it['pool_percentage']??0),2,',','.'))?>%<br><small>Budget <?=money_id($it['payroll_budget']??$it['pool_amount']??0)?></small></td><td><b><?=e(number_format((float)($it['kpi_score']??0),2,',','.'))?></b><br><small><?=e($it['kpi_source']??'')?><?=($it['unit_type']??'')==='dapur'?' · poin '.number_format((float)($it['kpi_raw']??0),2,',','.').' · faktor 25%' : ''?></small></td><td><?=e(number_format((float)($it['kpi_weight']??0)*100,2,',','.'))?>%</td><td><b><?=money_id($snap?($it['incentive_amount']??0):($it['incentive']??0))?></b></td><td><?=money_id($it['base_salary']??0)?></td><td><?=money_id($it['overtime']??0)?></td><td><?=money_id($it['bpjs_kesehatan']??0)?></td><td><?=money_id($it['bpjs_ketenagakerjaan']??0)?></td><td><?=money_id($it['punishment']??0)?></td><td><?=money_id($it['kasbon']??0)?></td><td><b><?=money_id($it['take_home']??0)?></b></td></tr><?php endforeach;if(!$payrollItems): ?><tr><td colspan="14">Belum ada pegawai eligible untuk payroll.</td></tr><?php endif; ?></tbody></table></div></div>
