...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try{
    ...
    if($action==='save_payroll_unit'){
      ...
      bo_exec("INSERT INTO bo_payroll_unit_settings(unit_type,system_key,unit_name,pool_percentage,is_active,created_at) VALUES(?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE unit_name=VALUES(unit_name),pool_percentage=VALUES(pool_percentage),is_active=VALUES(is_active),updated_at=NOW()",[$unitType,$systemKey,$unitName!==''?$unitName:$systemKey,$pct,isset($_POST['is_active'])?1:0]);
      bo_fin_redirect('payroll',$month,'Setting pool insentif disimpan.');
    }
    if($action==='save_payroll_employee'){
      ...
      $vals=[
        (float)str_replace(',','.',bo_fin_post('base_salary','0')),
        (float)str_replace(',','.',bo_fin_post('allowance','0')),
        (float)str_replace(',','.',bo_fin_post('overtime','0')),
        (float)str_replace(',','.',bo_fin_post('deduction','0')),
        (float)str_replace(',','.',bo_fin_post('adjustment','0')),
      ];
      if(min(array_slice($vals,0,4))<0) throw new RuntimeException('Gaji pokok, tunjangan, lembur, dan potongan tidak boleh negatif.');
      bo_exec("INSERT INTO bo_payroll_employee_settings(assignment_id,bo_user_id,base_salary,allowance,overtime,deduction,adjustment,is_eligible,notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE base_salary=VALUES(base_salary),allowance=VALUES(allowance),overtime=VALUES(overtime),deduction=VALUES(deduction),adjustment=VALUES(adjustment),is_eligible=VALUES(is_eligible),notes=VALUES(notes),updated_at=NOW()",[$assignmentId>0?$assignmentId:null,$boUserId>0?$boUserId:null,$vals[0],$vals[1],$vals[2],$vals[3],$vals[4],isset($_POST['is_eligible'])?1:0,bo_fin_post('notes')]);
      if($assignmentId>0 && bo_fin_post('unit_type')==='dapur'){
        ...
        if($role!==''&&$systemKey!==''&&$target>0) bo_exec("INSERT INTO bo_payroll_kpi_targets(unit_type,system_key,role_key,target_value,created_at) VALUES('dapur',?,?,?,NOW()) ON DUPLICATE KEY UPDATE target_value=VALUES(target_value),updated_at=NOW()",[$systemKey,$role,$target]);
      }
      bo_fin_redirect('payroll',$month,'Setting pegawai payroll disimpan.');
    }
    if($action==='save_admin_kpi'){
      $boUserId=(int)($_POST['bo_user_id']??0);$score=(float)str_replace(',','.',bo_fin_post('score','0'));
      if($boUserId<=0||$score<0||$score>120) throw new RuntimeException('KPI Admin harus 0–120.');
      $admin=bo_exec("SELECT id FROM bo_users WHERE id=? AND is_active=1 AND LOWER(role_key)='admin' LIMIT 1",[$boUserId])->fetch();if(!$admin)throw new RuntimeException('Admin Back Office tidak ditemukan.');
      bo_exec("INSERT INTO bo_payroll_admin_kpi(payroll_month,bo_user_id,score,notes,updated_by,created_at) VALUES(?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE score=VALUES(score),notes=VALUES(notes),updated_by=VALUES(updated_by),updated_at=NOW()",[$month,$boUserId,$score,bo_fin_post('notes'),$userId]);
      bo_fin_redirect('payroll',$month,'KPI Admin bulan '.$month.' disimpan.');
    }
...
<?php
$payrollReady=bo_payroll_schema_ready();$payrollUnits=$payrollReady?bo_payroll_unit_settings():[];$payrollEmpSettings=$payrollReady?bo_payroll_employee_settings():[];$payrollAdminKpis=$payrollReady?bo_payroll_admin_kpi_map($month):[];
...
$payrollLocked=$payrollRun&&in_array((string)$payrollRun['status'],['final','paid'],true);
$payrollCalc=null;if($payrollReady&&!$payrollLocked){try{$payrollCalc=bo_payroll_compute($month);}catch(Throwable $e){$payrollCalc=['items'=>[],'totals'=>['base_salary'=>0,'incentive'=>0,'allowance'=>0,'overtime'=>0,'deduction'=>0,'adjustment'=>0,'take_home'=>0],'kpi_errors'=>[$e->getMessage()],'financial_errors'=>[]];}}
$payrollItems=$payrollLocked?(array)($payrollRun['items']??[]):(array)($payrollCalc['items']??[]);
$payrollTotals=$payrollLocked?['base_salary'=>(float)$payrollRun['total_base_salary'],'incentive'=>(float)$payrollRun['total_incentive'],'allowance'=>(float)$payrollRun['total_allowance'],'overtime'=>(float)$payrollRun['total_overtime'],'deduction'=>(float)$payrollRun['total_deduction'],'adjustment'=>(float)$payrollRun['total_adjustment'],'take_home'=>(float)$payrollRun['total_take_home']]:(array)($payrollCalc['totals']??[]);
?>
<style>
.payroll-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px}.payroll-scroll{overflow:auto}.payroll-scroll table{min-width:1180px}.payroll-scroll input{min-width:105px}.payroll-status{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.payroll-unit-settings table{min-width:850px}.payroll-unit-label{text-transform:capitalize}.payroll-detail{font-size:11px;color:var(--muted)}
</style>
<?php if(!$payrollReady): ?><div class="alert danger">Tabel payroll belum tersedia. Jalankan migrasi SQL patch 20260828 terlebih dahulu.</div><?php else: ?>
<div class="card"><div class="section-head"><div><h3>Penggajian — <?=e($month)?></h3><div class="muted">Toko memakai omset toko + KPI toko; dapur memakai penjualan eksternal + nilai distribusi internal + KPI poin; Admin Back Office memakai omset konsolidasi eksternal + KPI Admin.</div></div><div class="payroll-status"><span class="badge <?=($payrollRun['status']??'live')==='paid'?'ok':(($payrollRun['status']??'live')==='final'?'warn':'')?>"><?=e(strtoupper((string)($payrollRun['status']??'LIVE')))?></span><?php if($payrollRun&&$payrollRun['status']==='final'): ?><form method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="mark_payroll_paid"><button class="btn primary" data-confirm="Tandai payroll ini sudah dibayar?">Tandai Dibayar</button></form><?php endif; ?></div></div></div>
<div class="payroll-grid section"><div class="card metric"><div class="label">Total Payroll</div><div class="value"><?=money_id($payrollTotals['take_home']??0)?></div></div><div class="card metric"><div class="label">Gaji Pokok</div><div class="value"><?=money_id($payrollTotals['base_salary']??0)?></div></div><div class="card metric"><div class="label">Insentif KPI</div><div class="value"><?=money_id($payrollTotals['incentive']??0)?></div></div><div class="card metric"><div class="label">Tunjangan + Lembur</div><div class="value"><?=money_id(($payrollTotals['allowance']??0)+($payrollTotals['overtime']??0))?></div></div><div class="card metric"><div class="label">Potongan</div><div class="value"><?=money_id($payrollTotals['deduction']??0)?></div></div></div>
<?php if(!$payrollLocked): ?>
<div class="card section payroll-unit-settings"><div class="section-head"><div><h3>Setting Pool Insentif per Unit</h3><div class="muted">Default 0%. Atur persentase sebelum finalisasi payroll.</div></div></div><div class="table-wrap"><table><thead><tr><th>Jenis</th><th>Unit</th><th>Basis Omset</th><th>% Pool</th><th>Status</th><th></th></tr></thead><tbody>
<?php foreach($payrollUnits as $u): $formId='pu-'.preg_replace('/[^a-zA-Z0-9_-]/','-',(string)$u['unit_type'].'-'.(string)$u['system_key']);$basis=0;if($payrollCalc){if($u['unit_type']==='store')$basis=(float)($payrollCalc['financial']['store'][$u['system_key']]['revenue']??0);elseif($u['unit_type']==='dapur')$basis=(float)($payrollCalc['financial']['dapur'][$u['system_key']]['revenue']??0);else $basis=(float)($payrollCalc['financial']['backoffice_revenue']??0);} ?><tr><td class="payroll-unit-label"><?=e($u['unit_type'])?><form id="<?=e($formId)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_unit"><input type="hidden" name="unit_type" value="<?=e($u['unit_type'])?>"><input type="hidden" name="system_key" value="<?=e($u['system_key'])?>"><input type="hidden" name="unit_name" value="<?=e($u['unit_name'])?>"></form></td><td><b><?=e($u['unit_name'])?></b><br><small><?=e($u['system_key'])?></small></td><td><?=money_id($basis)?></td><td><input form="<?=e($formId)?>" type="number" name="pool_percentage" min="0" max="100" step="0.01" value="<?=e($u['pool_percentage'])?>"></td><td><label><input form="<?=e($formId)?>" style="width:auto" type="checkbox" name="is_active" <?=!empty($u['is_active'])?'checked':''?>> Aktif</label></td><td><button form="<?=e($formId)?>" class="btn">Simpan</button></td></tr><?php endforeach; ?></tbody></table></div></div>
<div class="card section"><h3>Komponen Payroll Pegawai Toko & Dapur</h3><div class="muted">Gaji disimpan per assignment. Pegawai yang bekerja di dua unit dapat memiliki komponen berbeda di setiap unit.</div><div class="table-wrap payroll-scroll"><table><thead><tr><th>Pegawai</th><th>Unit / Role</th><th>Gaji Pokok</th><th>Tunjangan</th><th>Lembur</th><th>Potongan</th><th>Adjustment</th><th>Target KPI Dapur</th><th>Eligible</th><th></th></tr></thead><tbody>
<?php foreach($payrollAssignments as $a): $es=$payrollEmpSettings['assignment:'.(int)$a['assignment_id']]??[];$isDapur=(string)$a['source_system']==='dapur';$target=$isDapur?bo_payroll_target('dapur',(string)$a['system_key'],(string)$a['role_key']):100;$formId='pe-'.(int)$a['assignment_id']; ?><tr><td><b><?=e($a['canonical_name'])?></b><br><small><?=e($a['external_employee_id'])?></small><form id="<?=e($formId)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_employee"><input type="hidden" name="assignment_id" value="<?=e($a['assignment_id'])?>"><input type="hidden" name="unit_type" value="<?=$isDapur?'dapur':'store'?>"><input type="hidden" name="system_key" value="<?=e($a['system_key'])?>"><input type="hidden" name="role_key" value="<?=e($a['role_key'])?>"></form></td><td><?=e($a['system_name'])?><br><small><?=e($a['role_label']?:$a['role_key'])?></small></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1000" name="base_salary" value="<?=e($es['base_salary']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1000" name="allowance" value="<?=e($es['allowance']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1000" name="overtime" value="<?=e($es['overtime']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" min="0" step="1000" name="deduction" value="<?=e($es['deduction']??0)?>"></td><td><input form="<?=e($formId)?>" type="number" step="1000" name="adjustment" value="<?=e($es['adjustment']??0)?>"></td><td><?php if($isDapur): ?><input form="<?=e($formId)?>" type="number" min="0.01" step="1" name="kpi_target" value="<?=e($target)?>"><?php else: ?><span class="muted">KPI 0–100</span><?php endif; ?></td><td><input form="<?=e($formId)?>" style="width:auto" type="checkbox" name="is_eligible" <?=!isset($es['is_eligible'])||!empty($es['is_eligible'])?'checked':''?>></td><td><button form="<?=e($formId)?>" class="btn">Simpan</button></td></tr><?php endforeach;if(!$payrollAssignments): ?><tr><td colspan="10">Belum ada assignment pegawai aktif. Jalankan Sync Pegawai terlebih dahulu.</td></tr><?php endif; ?></tbody></table></div></div>
<div class="card section"><h3>Admin Back Office</h3><div class="muted">Hanya user dengan role <b>admin</b> yang masuk. Owner tidak dimasukkan otomatis.</div><div class="table-wrap payroll-scroll"><table><thead><tr><th>Admin</th><th>KPI <?=e($month)?></th><th>Gaji Pokok</th><th>Tunjangan</th><th>Lembur</th><th>Potongan</th><th>Adjustment</th><th>Eligible</th><th>Aksi</th></tr></thead><tbody>
<?php foreach($payrollAdmins as $a):$es=$payrollEmpSettings['admin:'.(int)$a['id']]??[];$ak=$payrollAdminKpis[(int)$a['id']]??[];$kForm='pk-'.(int)$a['id'];$gForm='pa-'.(int)$a['id']; ?><tr><td><b><?=e($a['name'])?></b><br><small><?=e($a['username'])?></small><form id="<?=e($kForm)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_admin_kpi"><input type="hidden" name="bo_user_id" value="<?=e($a['id'])?>"></form><form id="<?=e($gForm)?>" method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="save_payroll_employee"><input type="hidden" name="bo_user_id" value="<?=e($a['id'])?>"></form></td><td><div class="finance-actions"><input form="<?=e($kForm)?>" type="number" min="0" max="120" step="0.01" name="score" value="<?=e($ak['score']??0)?>"><button form="<?=e($kForm)?>" class="btn">KPI</button></div></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1000" name="base_salary" value="<?=e($es['base_salary']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1000" name="allowance" value="<?=e($es['allowance']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1000" name="overtime" value="<?=e($es['overtime']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" min="0" step="1000" name="deduction" value="<?=e($es['deduction']??0)?>"></td><td><input form="<?=e($gForm)?>" type="number" step="1000" name="adjustment" value="<?=e($es['adjustment']??0)?>"></td><td><input form="<?=e($gForm)?>" style="width:auto" type="checkbox" name="is_eligible" <?=!isset($es['is_eligible'])||!empty($es['is_eligible'])?'checked':''?>></td><td><button form="<?=e($gForm)?>" class="btn">Simpan Gaji</button></td></tr><?php endforeach;if(!$payrollAdmins): ?><tr><td colspan="9">Belum ada user role Admin aktif.</td></tr><?php endif; ?></tbody></table></div></div>
<?php endif; ?>
<div class="card section"><div class="section-head"><div><h3>Perhitungan Payroll</h3><div class="muted">Insentif unit = pool omset × proporsi KPI pegawai terhadap total KPI eligible dalam unit.</div></div><?php if(!$payrollLocked): ?><div class="finance-actions"><form method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="recalculate_payroll"><button class="btn">Simpan / Hitung Ulang Draft</button></form><form method="post"><input type="hidden" name="csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="finalize_payroll"><button class="btn primary" data-confirm="Finalisasi akan mengunci snapshot omset, KPI, formula dan nominal bulan ini. Lanjutkan?">Final & Kunci</button></form></div><?php endif; ?></div>
...
<div class="table-wrap payroll-scroll"><table><thead><tr><th>Pegawai</th><th>Unit</th><th>Basis Omset</th><th>Pool</th><th>KPI</th><th>Bobot Pool</th><th>Insentif</th><th>Gaji Pokok</th><th>Tunjangan</th><th>Lembur</th><th>Potongan</th><th>Adj.</th><th>Take Home</th></tr></thead><tbody>
<?php foreach($payrollItems as $it): $snap=$payrollLocked; ?><tr><td><b><?=e($snap?$it['employee_name_snapshot']:$it['employee_name'])?></b><br><small><?=e($snap?$it['role_key_snapshot']:$it['role_key'])?></small></td><td><?=e($snap?$it['unit_name_snapshot']:$it['unit_name'])?><br><small><?=e($it['unit_type'])?></small></td><td><?=money_id($it['revenue_base']??0)?></td><td><?=e(number_format((float)($it['pool_percentage']??0),2,',','.'))?>%<br><small><?=money_id($it['pool_amount']??0)?></small></td><td><b><?=e(number_format((float)($it['kpi_score']??0),2,',','.'))?></b><br><small><?=e($it['kpi_source']??'')?><?=($it['unit_type']??'')==='dapur'?' · '.number_format((float)($it['kpi_raw']??0),2,',','.').'/'.number_format((float)($it['kpi_target']??0),2,',','.') : ''?></small></td><td><?=e(number_format((float)($it['kpi_weight']??0)*100,2,',','.'))?>%</td><td><b><?=money_id($snap?($it['incentive_amount']??0):($it['incentive']??0))?></b></td><td><?=money_id($it['base_salary']??0)?></td><td><?=money_id($it['allowance']??0)?></td><td><?=money_id($it['overtime']??0)?></td><td><?=money_id($it['deduction']??0)?></td><td><?=money_id($it['adjustment']??0)?></td><td><b><?=money_id($it['take_home']??0)?></b></td></tr><?php endforeach;if(!$payrollItems): ?><tr><td colspan="13">Belum ada pegawai eligible untuk payroll.</td></tr><?php endif; ?></tbody></table></div></div>
<?php if($payrollLocked): ?><div class="alert section">Payroll <?=e($month)?> sudah <b><?=e(strtoupper($payrollRun['status']))?></b>. Snapshot omset, KPI, formula dan nominal tidak lagi mengikuti perubahan data sumber.<?php if(!empty($payrollRun['finalized_at'])): ?> Finalisasi: <?=e($payrollRun['finalized_at'])?>.<?php endif; ?><?php if(!empty($payrollRun['paid_at'])): ?> Dibayar: <?=e($payrollRun['paid_at'])?>.<?php endif; ?></div><?php endif; ?>
<?php endif; ?>
