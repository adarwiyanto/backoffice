...
require_once __DIR__.'/../core/BackupAdapter.php'; require_once __DIR__.'/backup_ui.php'; $svc=bo_backup_service(); $msg=(string)($_SESSION['bo_backup_flash']??'');$err=(string)($_SESSION['bo_backup_flash_error']??'');unset($_SESSION['bo_backup_flash'],$_SESSION['bo_backup_flash_error']);
...
$cronCommand='/usr/local/bin/php -q '.escapeshellarg(dirname(__DIR__).'/cron_backup.php').' >/dev/null 2>&1';$relativeCron=bo_url('cron_backup.php?key='.rawurlencode((string)$svc->get('cron_secret','')));$cronUrl=preg_match('~^https?://~i',$relativeCron)?$relativeCron:$scheme.'://'.($_SERVER['HTTP_HOST']??'localhost').'/'.ltrim($relativeCron,'/');
?><div class="page-title"><div><h1>Setting Backup Google Drive</h1><div class="muted">Backup otomatis terenkripsi untuk Back Office.</div></div></div><?php if($msg):?><div class="alert success"><?=e($msg)?></div><?php endif;?><?php if($err):?><div class="alert danger"><?=e($err)?></div><?php endif;?><?php backup_render_settings($svc,$callback,$cronCommand,$cronUrl,'<input type="hidden" name="backup_csrf" value="'.e($csrf).'">','?p=settings'); ?>
