...
if($svc){$cronCommand=backup_build_cron_command($svc,dirname(__DIR__).'/cron_backup.php');$relativeCron=bo_url('cron_backup.php?key='.rawurlencode((string)$svc->get('cron_secret','')));$cronUrl=preg_match('~^https?://~i',$relativeCron)?$relativeCron:$scheme.'://'.($_SERVER['HTTP_HOST']??'localhost').'/'.ltrim($relativeCron,'/');backup_render_settings($svc,$callback,$cronCommand,$cronUrl,'<input type="hidden" name="backup_csrf" value="'.e($csrf).'">','?p=settings');}
